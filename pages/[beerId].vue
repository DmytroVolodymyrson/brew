<template>
  <div class="card mx-auto max-w-2xl shadow-xl">
    <div><img :src="beer.image_url" alt="Beer image" class="h-64 w-full object-contain" /></div>
    ...
    <div class="space-y-4 p-6">
      ...
      <div class="flex flex-wrap items-start gap-x-4 gap-y-3">
        ...
        >
          <h5 class="font-bold">Boil Volume:</h5>
        ...
        <ul :class="['grid flex-wrap items-start gap-4 sm:grid-cols-2', mashIsMoreThanOneStep ? 'mt-4' : 'mt-2']">
          <li
            ...
            :class="[
              ...
              {
                'rounded border px-4 py-2': mashIsMoreThanOneStep,
              },
            ]"
          >
            <div
    ...
    <div v-if="beer.ingredients" class="p-6">
      ...
      <div v-if="beer.ingredients.hops && beer.ingredients.hops.length > 0" class="my-5">
        ...
        <div class="grid gap-2 sm:grid-cols-2 sm:gap-4">
          <div v-for="hop in beer.ingredients.hops" :key="hop.name" class="rounded-md border-2 border-gray-200 p-3">
            <p v-if="hop.name"><span class="font-bold">Name:</span> {{ hop.name }}</p>
            <p v-if="hop.amount"><span class="font-bold">Amount:</span> {{ hop.amount.value }} {{ hop.amount.unit }}</p>
...
const mashIsMoreThanOneStep = beer.value.method.mash_temp.length > 1
</script>
